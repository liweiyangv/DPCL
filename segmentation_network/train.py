...
SNAPSHOT_DIR = '/public/home/yangliwei/DG_project/snapshots/disturb_recon_ori_with_alter_mean_var/total_method_crop_size_bs8_40000_le_3/'
VISUAL_RESULT = '/public/home/yangliwei/DG_project/result/disturb_recon_ori_with_alter_mean_var/total_method_crop_size_bs8_40000_le_3/'
PRE_RESULT = '/public/home/yangliwei/DG_project/pre_result/prototype_classifer/total_method_crop_size_bs8_40000_le_3/'
...
parser.add_argument('--date', type=str, default='2022_8_2_619',
                    help='experiment directory date name')
parser.add_argument('--exp', type=str, default='total_method_crop_size_bs8_40000_le_3',
...
parser.add_argument("--marginal_param", type=float, default=0.5, help="marginal parameter in instance-to-class contrast")
parser.add_argument("--hard_pixel_prop", type=str, default=0.5, help="hard pixel proportion")
parser.add_argument("--moving_param", type=float, default=0.999, help=" parameter in class prototype update")
parser.add_argument("--big_moving_param", type=float, default=0.7, help=" parameter in class prototype update for first step")
...
def main():
    ...
    if args.use_warmup:
        ...
        seg_checkpoint = torch.load(
            '/public/home/yangliwei/DG_project/snapshots/disturb_recon_ori_with_alter_mean_var/6_lr_1e_3_7000_warmup/GTA5_only_seg_epoch_9.pth')
        model_transfer.load_state_dict(torch.load(
            '/public/home/yangliwei/DG_project/snapshots/disturb_recon_ori_with_alter_mean_var/6_lr_1e_3_7000_warmup/GTA5_only_transfer_epoch_9.pth'))
        net = optimizer.forgiving_state_restore(net, seg_checkpoint)
        optim_old = torch.load(
            '/public/home/yangliwei/DG_project/snapshots/disturb_recon_ori_with_alter_mean_var/6_lr_1e_3_7000_warmup/optim_epoch_9.pth')
        optim.load_state_dict(optim_old)
        scheduler.load_state_dict(torch.load(
            '/public/home/yangliwei/DG_project/snapshots/disturb_recon_ori_with_alter_mean_var/6_lr_1e_3_7000_warmup/scheduler_epoch_9.pth'))
        optimizer_transfer.load_state_dict(torch.load(
            '/public/home/yangliwei/DG_project/snapshots/disturb_recon_ori_with_alter_mean_var/6_lr_1e_3_7000_warmup/optimizer_transfer_epoch_9.pth'))
        epoch = 10
    else:
        print('do not warm up------------------------------------------------------------------------------------------')
        model_transfer.load_state_dict(torch.load(args.pretrained_autoencoder))
        print('ae path',args.pretrained_autoencoder)
            ...
            if curr_epoch > args.warmup_end_epoch:
                ...
                for label_id in np.arange(class_num):
                    ...
                    if sum > 0:
                        ...
                        current_pixel_feature = random_select_half_hard_pixel(feature_clone_2,easy_index,hard_index,args.random_select_num,args.hard_pixel_prop)
                ...
                if curr_iter < args.num_big_momentum + (args.warmup_end_epoch + 1) * len(train_loader) + 1:
                    ...
                    class_prototype[index_update, :] = args.big_moving_param * old_prototype[index_update, :] + (1-args.big_moving_param) * new_class_prototype[index_update, :]
                else:
                    if curr_epoch == args.warmup_end_epoch + 1:
                        ...
                        class_prototype[index_big_update, :] = args.big_moving_param * old_prototype[index_big_update,:] + (1-args.big_moving_param) * new_class_prototype[index_big_update, :]
                        class_prototype[index_small_update, :] = args.moving_param * old_prototype[index_small_update,:] + (1-args.moving_param) * new_class_prototype[index_small_update, :]
                    else:
                        index_update = np.where(current_update_index > 0, True, False)  # update index
                        class_prototype[index_update, :] = args.moving_param * old_prototype[index_update,:] + (1-args.moving_param) * new_class_prototype[index_update, :]
