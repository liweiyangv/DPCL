...
from autoencoder.cyc_autoencoder_old import AutoEncoder
...
PRETRAINED_AUTOENCODER = '/media/yangliwei/lemon/fixed_point/snapshots/ae/pretrained/alter_mean_var_in_second_layer/noise_color_0_2_0_2_0_2_0_1/alter_after_norm2d/GTA5_only_transfer_105000.pth'
SNAPSHOT_DIR = '/media/yangliwei/lemon/fixed_point/snapshots/prototype_classifer/cityscapes/res50_iter_ours_step_15_threshold_0_1000_forth/'
VISUAL_RESULT = '/media/yangliwei/lemon/fixed_point/result/prototype_classifer/cityscapes/res50_iter_ours_step_15_threshold_0_1000_forth/'
PRE_RESULT = '/media/yangliwei/lemon/fixed_point/pre_result/prototype_classifer/cityscapes/res50_iter_ours_step_15_threshold_0_1000_forth/'
...
parser.add_argument('--arch', type=str, default='network.deepv3_with_prototype_test_time.DeepR50V3PlusD',
...
parser.add_argument('--date', type=str, default='2022_8_15',
                    help='experiment directory date name')
parser.add_argument('--exp', type=str, default='res50_iter_ours_step_15_threshold_0_1000_forth',
...
def main():
    ...
    if args.use_warmup:
        ...
        # model_transfer.load_state_dict(torch.load(args.pretrained_autoencoder),map_location={'cuda:0':'cuda:1'})
...
def cal_test_contrastive_loss(net,image,prototype,threshold):
    ...
    for label_id in np.arange(19):
        ...
        if torch.sum(update_id) > 5:
            ...
            select_feature_normalize = random_select_pixel(current_feature,1000)
    ...
    return loss_contrast,prototype
...
def validate(val_loader, dataset, net, criterion, optim, scheduler, curr_epoch, writer, curr_iter, model_transfer, optimizer_transfer,class_prototype,save_pth=True):
    ...
    mean_data = torch.load(
        '/media/yangliwei/lemon/code/robustnet_with_fixed_point/test_iter_contrast/mean_center_12388.pt').cuda(args.gpu)
    var_data = torch.load(
        '/media/yangliwei/lemon/code/robustnet_with_fixed_point/test_iter_contrast/var_center_12388.pt').cuda(args.gpu)
    ...
    for val_idx, data in enumerate(val_loader):
        ...
        if args.use_wtloss:
            ...
            test_contrastive_loss,class_prototype_clone = cal_test_contrastive_loss(net, project_image,class_prototype_clone,0.)
            grad_contrastive = torch.autograd.grad(test_contrastive_loss,project_image)
            iter_image_1 = project_image - 15 * grad_contrastive[0]
            iter_image_1 = iter_image_1.detach()
            del grad_contrastive,test_contrastive_loss,project_image,recon_feature,image_transform_encode,image_transform_encode_mean_,image_transform_encode_mean
            # del project_image,grad_contrastive,test_contrastive_loss
            torch.cuda.empty_cache()
            ...
            with torch.no_grad():
                ...
                distance_softmax = torch.softmax(distance, dim=0).reshape(b, 19, x, y)
                output = torch.mean(torch.cat((classifier_prob_low, distance_softmax), dim=0), dim=0, keepdim=True)
                output = torch.nn.functional.interpolate(output, size=classifier_output.size()[2:], mode='bilinear',
                                                         align_corners=True)
                distance_prob = torch.nn.functional.interpolate(distance_softmax, size=classifier_output.size()[2:],
        ...
        if val_idx % 1 == 0:
            if args.local_rank == 0:
                ...
                output_col.save('%s/epoch_%s/%s_mean_output.png' % (args.pre_result, str(curr_epoch),str(img_names)))
                #
                ...
                # classifier_prob_output_col.save('%s/epoch_%s/%s_classifier_output.png' % (args.pre_result, str(curr_epoch), str(img_names)))
                #
                ...
                # pred_prototype_output_col.save('%s/epoch_%s/%s_prototype_output.png' % (args.pre_result, str(curr_epoch), str(img_names)))

                gt_show = gt_image.cpu().data[0].numpy()
                gt_show = np.asarray(gt_show, dtype=np.uint8)
                gt_show_col = colorize_mask(gt_show)
                gt_show_col.save('%s/epoch_%s/%s_gt.png' % (args.pre_result,str(curr_epoch),str(img_names)))
