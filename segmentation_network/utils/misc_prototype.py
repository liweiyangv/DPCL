...
def fast_hist(label_pred, label_true, num_classes):
    ...
    a = num_classes * label_true[mask].astype(int)
    b = label_pred[mask]
    hist = np.bincount(
...
def evaluate_eval(args, net, optimizer, scheduler, val_loss, hist, dump_images, writer, epoch=0, dataset_name=None, dataset=None, curr_iter=0, optimizer_at=None, scheduler_at=None, save_pth=True,model_transfer=None,optimizer_transfer=None,class_prototype=None):
    """
    Modified IOU mechanism for on-the-fly IOU calculations ( prevents memory overflow for
    large dataset) Only applies to eval/eval.py
    """
