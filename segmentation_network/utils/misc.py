...
def evaluate_eval(args, net, optimizer, scheduler,scheduler_transfer, val_loss, hist, dump_images, writer, epoch=0, dataset_name=None,
                  dataset=None, curr_iter=0, optimizer_at=None, scheduler_at=None, save_pth=True, model_transfer=None,
                  optimizer_transfer=None, kmeans_centers=None, kmeans_centers_mean_var=None):
    """
    Modified IOU mechanism for on-the-fly IOU calculations ( prevents memory overflow for
    large dataset) Only applies to eval/eval.py
    """
        ...
        if optimizer_at is not None:
            if model_transfer is not None:
                torch.save({
                    ...
                    'scheduler_transfer': scheduler_transfer.state_dict(),                  
                    'scheduler_at': scheduler_at.state_dict(),
                    'model_transfer': model_transfer.state_dict(),
                    'optimizer_transfer': optimizer_transfer.state_dict(),
                    'kmeans_centers': kmeans_centers,
                    'kmeans_centers_mean_var': kmeans_centers_mean_var,
                    'epoch': epoch,
                    'mean_iu': mean_iu,
    ...
    if val_loss is not None and hist is not None:
        ...
        if save_pth:
            ...
            logging.info(fmt_str % (dataset_name,
                                    ...
                                    args.best_record[dataset_name]['acc_cls'],
                                    args.best_record[dataset_name]['mean_iu'],
