...
PRETRAINED_AUTOENCODER = '/media/omnisky/Disk2/yangliwei/experiment_data/fixed_point/splitted/snapshots/ae/cityscapes/0_2_0_2_0_2_0_1_noise/GTA5_only_transfer_120000.pth'
SNAPSHOT_DIR = '/media/omnisky/Disk2/yangliwei/experiment_data/fixed_point/splitted/snapshots/prototype_classifer/total_cityscapes_120000ae_lr_51e_3_50000_bs8_mobilenet_2/'
VISUAL_RESULT = '/media/omnisky/Disk2/yangliwei/experiment_data/fixed_point/splitted/result/prototype_classifier/total_cityscapes_120000ae_lr_51e_3_50000_bs8_mobilenet_2/'
PRE_RESULT = '/media/omnisky/Disk2/yangliwei/experiment_data/fixed_point/splitted/pre_result/prototype_classifier/total_cityscapes_120000ae_lr_51e_3_50000_bs8_mobilenet_2/'
...
parser.add_argument('--arch', type=str, default='network.deepv3_with_prototype_dec2.DeepMobileNetV3PlusD',
...
parser.add_argument('--max_iter', type=int, default=55000)
...
parser.add_argument('--date', type=str, default='2022_8_12_619',
                    help='experiment directory date name')
parser.add_argument('--exp', type=str, default='total_cityscapes_120000ae_lr_51e_3_50000_bs8_mobilenet_2',
...
parser.add_argument("--num_big_momentum", type=int, default=200, help="number of centor in k-means ")
...
parser.add_argument("--random_select_num", type=int, default=50, help="number of select pixel in pixel_contrast")
...
def main():
    ...
    if args.use_warmup:
        ...
        seg_checkpoint = torch.load(
            '/media/omnisky/Disk2/yangliwei/experiment_data/fixed_point/splitted/snapshots/prototype_classifer/total_method_cityscapes_5_1e_3_bs8_50000_12wae/GTA5_only_seg_epoch_9.pth',map_location='cpu')
        ...
        old_optim =torch.load(
            '/media/omnisky/Disk2/yangliwei/experiment_data/fixed_point/splitted/snapshots/prototype_classifer/total_method_cityscapes_5_1e_3_bs8_50000_12wae/optim_epoch_9.pth',map_location='cpu')
        optim.load_state_dict(old_optim)

        # optim.param_groups[0]['lr'] = old_optim['param_groups'][0]['lr']
        scheduler.load_state_dict(torch.load(
            '/media/omnisky/Disk2/yangliwei/experiment_data/fixed_point/splitted/snapshots/prototype_classifer/total_method_cityscapes_5_1e_3_bs8_50000_12wae/scheduler_epoch_9.pth',map_location='cpu'))
    ...
    while i < args.max_iter:
        ...
        cfg.ITER = i
        cfg.immutable(True)
        i,class_prototype,cal_update_num = train(train_loader, net, model_transfer,optim, optimizer_transfer,epoch, writer, scheduler,class_prototype,cal_update_num,weighted_matrix, args.max_iter,extra_val_loaders,criterion_val)
...
def train(train_loader, net, model_transfer, optim, optimizer_transfer, curr_epoch, writer,scheduler,class_prototype, cal_update_num,weighted_matrix,max_iter,extra_val_loaders,criterion_val):
    """
    ...
    for i, data in enumerate(train_loader):
        ...
        for di, ingredients in enumerate(zip(ori_image,inputs, gts, aux_gts)):
            ...
            if curr_epoch > args.warmup_end_epoch:
                ...
                instance_pic_memory_contrastive_loss = instance_triplet_loss(instance_protoype, prototype_normalize, 1)
                ...
                if curr_iter < args.num_big_momentum + (args.warmup_end_epoch + 1) * len(train_loader) + 1:
                    ...
                    if curr_epoch == args.warmup_end_epoch + 1:
                        ...
                        class_prototype[index_small_update, :] = 0.99 * old_prototype[index_small_update,:] + 0.01 * new_class_prototype[index_small_update, :]
                    else:
                        index_update = np.where(current_update_index > 0, True, False)  # update index
                        class_prototype[index_update, :] = 0.99 * old_prototype[index_update,:] + 0.01 * new_class_prototype[index_update, :]
...
def validate(val_loader, dataset, net, criterion, optim, scheduler, curr_epoch, writer, curr_iter, model_transfer, optimizer_transfer,class_prototype,save_pth=True):
    ...
    for val_idx, data in enumerate(val_loader):
        ...
        if val_idx % 20 == 0:
            if args.local_rank == 0:
                ...
                vutils.save_image(project_image, '%s/project_%s.png'%(args.pre_result,str(val_idx)), normalize=True)
    ...
    if args.gpu == args.gpu:
        evaluate_eval(args, net, optim, scheduler,val_loss, iou_acc, dump_images,
                    ...
                    save_pth=True,model_transfer=model_transfer,optimizer_transfer=optimizer_transfer,class_prototype=class_prototype)
