...
def add_items(items, aug_items, cities, img_path, mask_path, mask_postfix, mode, maxSkip):
    ...
    for c in cities:
        ...
        c_items = sorted([name.split(img_postfix)[0] for name in
...
class GTAV(data.Dataset):
    ...
    def __init__(self, mode, maxSkip=0, joint_transform=None, sliding_crop=None,
        ...
        self.mean_std = ([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])
    ...
    def __getitem__(self, index):
        ...
        if img.size == mask.size:
            ...
            # if img.size[0]%4==0:
            #     mask = np.array(mask)  # Image.open(mask_path)
            #     img_name = os.path.splitext(os.path.basename(img_path))[0]
            # else:
            x,y=img.size
            delta_x = 4*(int(x/4)+1)-x
            delta_y = 4*(int(y/4)+1)-y
            border = (int(delta_x/2), int(delta_y/2), int(delta_x/2), int(delta_y/2))
            ...
            img_name = os.path.splitext(os.path.basename(img_path))[0]
            print(img_name)
            while img.size != mask.size:
                print("Error!!", img.size, mask.size, img_name)
        ...
        if self.eval_mode:
            print('here is gtav eval mode')
            rgb_mean_std_gt = ([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])
            img = transforms.Normalize(*rgb_mean_std_gt)(transforms.ToTensor()(img))
        ...
        return img, mask, img_name, mask_aux,border
...
class GTAVUniform(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if img.size == mask.size:
            ...
            mask = np.array(mask)
            img_name = os.path.splitext(os.path.basename(img_path))[0]
            image_transform = copy.deepcopy(img)
        else:
            img_name = os.path.splitext(os.path.basename(img_path))[0]
        # print(img.size, mask[:,:,0].shape)
            while img.size != mask.size:
                print("Error!!", img.size, mask.size, img_name)
                print("Dropping gtavuniform", str(index))
                # index = index + 1
                if index + 1 == len(self.imgs):
                    index = 0
                else:
        ...
        # train_input_transform += [RandomGaussianNoise()]
        ...
        image_transform = train_input_transform(image_transform)
        rgb_mean_std_gt = ([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])
        img = transforms.Normalize(*rgb_mean_std_gt)(transforms.ToTensor()(img))
        ...
        return img,image_transform, mask, img_name, mask_aux
...
class GTAVAug(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_or = transforms.Normalize(*rgb_mean_std_gt)(img_or)
        img_color = transforms.Normalize(*rgb_mean_std_gt)(img_color)
        img_geometric = transforms.Normalize(*rgb_mean_std_gt)(img_geometric)
