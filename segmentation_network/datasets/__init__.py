...
def setup_loaders(args):
    ...
    for i, val_set in enumerate(val_sets):
        if args.syncbn:
            # val_sampler = torch.utils.data.DataLoader(val_set,)
            val_sampler = DistributedSampler(val_set, pad=False, permutation=False, consecutive_sample=False)
    ...
    train_loader = DataLoader(train_set,batch_size=args.BATCHSIZE, shuffle=True, num_workers=args.num_workers, pin_memory=True,drop_last=True)
    extra_val_loader = {}
