...
class DomainUniformConcatDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for dataset in self.datasets:
            img, image_transform,mask, img_name, mask_aux = self._get_batch_from_dataset(dataset, idx)
            imgs.append(img)
            image_transforms.append(image_transform)
            masks.append(mask)
            img_names.append(img_name)
            mask_auxs.append(mask_aux)
        imgs,image_transforms, masks, mask_auxs = torch.stack(imgs, 0),torch.stack(image_transforms,0), torch.stack(masks, 0), torch.stack(mask_auxs, 0)

        return imgs,image_transforms, masks, img_names, mask_auxs
