...
def get_optimizer_transfer(args, net):
    """
    Decide Optimizer (Adam or SGD)
    """
    base_params = []
...
def load_weights(net, optimizer, scheduler,snapshot_file, restore_optimizer_bool=False,model_transfer=None,optimizer_transfer=None,class_prototype=None):
    """
    Load weights from snapshot file
    """
    logging.info("Loading weights from model %s", snapshot_file)
    net, optimizer, scheduler, epoch, mean_iu,model_transfer,optimizer_transfer,class_prototype = restore_snapshot(net, optimizer, scheduler,
                                                                                                                            snapshot_file,restore_optimizer_bool,model_transfer,optimizer_transfer,class_prototype)
    return net, optimizer, scheduler, epoch, mean_iu,model_transfer,optimizer_transfer,class_prototype
...
def restore_snapshot(net, optimizer,scheduler,snapshot, restore_optimizer_bool,model_transfer=None,optimizer_transfer=None,class_prototype=None):
    """
    ...
    if optimizer_transfer is not None and 'optimizer' in checkpoint and restore_optimizer_bool:
        optimizer_transfer.load_state_dict(checkpoint['optimizer_transfer'])
    if scheduler is not None and 'scheduler' in checkpoint and restore_optimizer_bool:
        scheduler.load_state_dict(checkpoint['scheduler'])
    if model_transfer is not None and 'model_transfer' in checkpoint:
        model_transfer.load_state_dict(checkpoint['model_transfer'])

    if class_prototype is not None and 'class_prototype' in checkpoint:
        class_prototype = checkpoint['class_prototype']
    ...
    return net, optimizer, scheduler, checkpoint['epoch'], checkpoint['mean_iu'],model_transfer,optimizer_transfer,class_prototype
