    ...
    def __init__(self, input_nc=3, output_nc=3, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        ...
        for i in range(n_downsampling):  # add downsampling layers
            ...
            if i==0:
                 ...
                 model_decoder += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                      norm_layer(ngf * mult * 2),
                      nn.ReLU(True)]
        ...
        for i in range(n_blocks):       # add ResNet blocks
            print(padding_type,norm_layer,use_bias)
            model_decoder += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=False, use_bias=use_bias)]
