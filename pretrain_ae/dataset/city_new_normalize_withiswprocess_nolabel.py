...
class cityDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321), scale=True, mirror=True, ignore_label=255):
        ...
        city_img_dir = '/public/home/yangliwei/dataset/cityscapes/leftImg8bit_trainvaltest/leftImg8bit/train/'
        # for split in ["train", "trainval", "val"]:
        for city_name in os.listdir(city_img_dir):
               current_path = city_img_dir + city_name + '/'
               for name in os.listdir(current_path):
                  img_file = osp.join('/public/home/yangliwei/dataset/cityscapes/leftImg8bit_trainvaltest/leftImg8bit/train/%s/%s'% (city_name,name))
                  self.files.append({
                      "img": img_file,
                      "name": name
                  })
