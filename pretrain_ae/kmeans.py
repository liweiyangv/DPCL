...
class KMeans:
  ...
  def predict(self, X,centroids):
    ...
    batch_size, emb_dim = X.shape
    if centroids is None:
      centor = X[np.random.choice(batch_size, size=[self.n_clusters], replace=False)]
    else:
      centor = centroids.cuda()
    return self.max_sim(a=X, b=centor)[1],centor
  ...
  def fit(self, X, centroids=None):
    """
      ...
      X: torch.Tensor, shape: [n_samples, n_features]
    """
    return self.fit_predict(X, centroids)